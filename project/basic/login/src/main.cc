...
bool LoginApp::Initialize() {
  static LoginServiceConfig config;
  static LoginServiceContext ctx(config);
  ...
  config.schema_bname = miku::cfg_get_s(conf, "schema_base_name", "login");
  config.table_bname = miku::cfg_get_s(conf, "table_base_name", "login");


  // grpc
  auto starget = miku::cfg_get_s(conf, "service.target", "0.0.0.0:50082");
  auto sname = miku::cfg_get_s(conf, "service.name", "user_service");

  RegisterService({sname, starget}, new LoginServiceImpl(ctx));
  ...
  for (uint32_t i = 0; i < LoginServiceContext::schema_count; i++) {
    auto [proxy, sidx, tidx] = ctx.mysql_router.GetProxy(
             i * LoginServiceContext::table_count);
  ...
  auto hsport = miku::cfg_get_ui(conf, "http_service.port", 5211);
  ...
  RegisterHttpService(HTTP_HELP_GRPC_SERVICE_SV(miku::login, interface),
      {hshost, hsport, hsparallel, hstimeout, hscert_file, hskey_file});

  CLIENT_HELP_INITIALIZE(Login)({
      "", {miku::cfg_get_s(conf, "client.self.target", "127.0.0.1:50082")}});

  RequestMap(HTTP_HELP_GRPC_REGISTER_CB, Login, miku::login, interface);
